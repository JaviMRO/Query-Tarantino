...
RUNS=(0 1 2 3)
STRUCTURES=(time book batch)

OUTPUT_FILE="${RESULTS_DIR}/${LANGUAGE}_datalake.csv"
...
main() {
    ...
    for structure in "${STRUCTURES[@]}"; do
        for n_books in "${N_BOOKS[@]}"; do
            for run in "${RUNS[@]}"; do
                run_one "${structure}" "${n_books}" "${run}"
            done
        done
    done

    echo
    echo "Datalake benchmarks completed."
